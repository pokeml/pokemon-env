...
class Pokemon {
    ...
    getTypes() {
        ...
        if (this.volatiles.typechange) {
            ...
            types = (window.BattleTeambuilderTable &&
...
class Battle {
    ...
    setTurn(turnNum) {
        ...
        if (this.turn && !this.usesUpkeep)
            this.updatePseudoWeatherLeft(); // for compatibility with old replays
        this.turn = turnNum;
                    ...
                    if (kwargs.silent) {
                        ...
                        switch (effect.id) {
                            case 'bellydrum':
                                ...
                                break;
                            case 'angerpoint':
                                this.scene.runOtherAnim('anger', [poke]);
    ...
    */
        ...
        switch (args[0]) {
            ...
            case 'c:': {
                ...
                if (this.ignoreOpponent && (rank === '\u2605' || rank === '\u2606') && toUserid(name) !== app.user.get('userid'))
                    break;
                if (window.app && app.ignore && app.ignore[toUserid(name)] && (rank === ' ' || rank === '+' || rank === '\u2605' || rank === '\u2606'))
                    break;
    ...
    nextActivity() {
        ...
        if (this.playbackState === Playback.Paused)
            return;
